...
if __name__ == '__main__':
    ...
    model.train_predict(KNeighborsClassifier(n_neighbors=2, weights='distance'), '2 Neighbors', X, Y)
    model.train_predict(SVC(kernel='poly', gamma=0.01, C=1.0), 'SVC(RBF)Classifier', X, Y)
    model.train_predict(SVC(kernel='linear', C=0.3), 'SVC(Linear)Classifier', X, Y)
    model.train_predict(MLPClassifier(algorithm='adam', alpha=1e-5, hidden_layer_sizes=(100,), activation='relu', random_state=1, learning_rate='constant'), 'MultiLayer BP', X, Y)
